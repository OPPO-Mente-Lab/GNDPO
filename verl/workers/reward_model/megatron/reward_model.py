...
class MegatronRewardModel(BasePPORewardModel):
    ...
    def compute_reward(self, data: DataProto) -> DataProto:
        ...
        # On-policy distillation reward: negative KL divergence between current and teacher model per token
        # batch["rollout_batch_logits"]: (bs, seq_len, vocab_size), current model's logits for each token
        # logits: (bs, seq_len, vocab_size), teacher model's logits for each token; here 'logits' is the output of this model

        if self.config.reward_model.on_policy_distill and "rollout_batch_logits" in data.batch:
            # Compute per-token KL divergence: KL(teacher || current)
            # formula: KL = log p_teacher - log p_current (evaluated at generated token)
            teacher_logits = logits  # (bs, seq_len, vocab_size)
            student_logits = data.batch["rollout_batch_logits"]  # (bs, seq_len, vocab_size)

            # Mask: only compute KL for response tokens. responses is (bs, response_length)
            # Rollout log prob shapes match responses, so use response_length
            # We broadcast/mask everything to shape (bs, response_length)
            bs, response_length = responses.shape

            # for each token, get the id to index into logits
            # Typically, teacher/student_logits have all tokens (prompt + response), but in most distill settings
            # we want response tokens only, so get their positions
            logp_teacher = torch.log_softmax(teacher_logits[:, -response_length:, :], dim=-1)  # (bs, response_length, vocab_size)
            logp_student = torch.log_softmax(student_logits[:, -response_length:, :], dim=-1)  # (bs, response_length, vocab_size)

            # Gather log_probs at generated tokens
            gather_idx = responses.unsqueeze(-1)  # (bs, response_length, 1)
            logp_teacher_token = torch.gather(logp_teacher, dim=-1, index=gather_idx).squeeze(-1)  # (bs, response_length)
            logp_student_token = torch.gather(logp_student, dim=-1, index=gather_idx).squeeze(-1)  # (bs, response_length)

            # KL per token: (log p_teacher - log p_student)
            # Reward often taken as negative KL (the higher, the better imitation)
            kl_per_token = logp_teacher_token - logp_student_token  # (bs, response_length)
            token_level_rewards = -kl_per_token  # (bs, response_length), higher reward = smaller divergence
        # 是否需要归一化 (normalization) 奖励要根据任务而定。
        # 通常对于 reward model 输出的打分，如果上线下游 RL 场景如 PPO 时，
        # 会在 RL pipeline 的 advantage/reward 处理中做归一化。
        # 但 reward model 本身一般不需要在输出 (token_level_rewards) 归一化，
        # 直接输出原始 rewards，归一化应交由后续 RL 流程处理。
        # 如果特定需求，需要归一化可以如下操作（仅供参考，默认不做）:
        # token_level_rewards = (token_level_rewards - token_level_rewards.mean()) / (token_level_rewards.std() + 1e-8)
        else:
