...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _build_model(self, config):
        ...
        from transformers import AutoConfig, AutoModelForTokenClassification, AutoModelForCausalLM, AutoModelForVision2Seq, AutoModelForImageTextToText
        ...
        trust_remote_code = config.model.get("trust_remote_code", False)
        if config.on_policy_distill:
            reward_model_config = AutoConfig.from_pretrained(local_path, trust_remote_code=trust_remote_code, attn_implementation="flash_attention_2")
        else:
        ...
        # note that we have to create model in fp32. Otherwise, the optimizer is in bf16, which is incorrect
        init_context = get_init_weight_context_manager(
            use_meta_tensor=not reward_model_config.tie_word_embeddings, mesh=self.device_mesh
        )

        with init_context(), warnings.catch_warnings():
            if config.on_policy_distill:
                warnings.simplefilter("ignore")
                if type(reward_model_config) in AutoModelForVision2Seq._model_mapping.keys():
                    reward_module_class = AutoModelForVision2Seq
                elif type(reward_model_config) in AutoModelForImageTextToText._model_mapping.keys():
                    reward_module_class = AutoModelForImageTextToText
                else:
                    reward_module_class = AutoModelForCausalLM
                model_init_kwargs = dict(attn_implementation="flash_attention_2")
                if re.match("internvl", reward_model_config.model_type, re.IGNORECASE):
                    if "flash_attention_2" in model_init_kwargs.get("attn_implementation"):
                        model_init_kwargs.pop("attn_implementation")
                        model_init_kwargs["use_flash_attn"] = True
                ...
                if re.match("internvl", reward_module.config.model_type):
                    #Frozen the vision parameters for InternVL. We need to find the reason why internvl will occur a bug when using the vision encoder
                    reward_module.img_context_token_id = self.tokenizer.convert_tokens_to_ids(self.tokenizer.context_image_token)
    ...
    def _forward_micro_batch(self, micro_batch):
        ...
        if self.config.on_policy_distill:
            ...
            if "multi_modal_inputs" in micro_batch.keys():
                if "image_bound" in micro_batch["multi_modal_inputs"][0]:  # minicpm-o logic
                    ...
                    for key in micro_batch["multi_modal_inputs"][0].keys():
                        ...
                        if re.match("internvl", self.reward_module.config.model_type):
            ...
            with torch.no_grad(), torch.autocast(device_type=device_name, dtype=torch.bfloat16):
                ...
                if self.use_remove_padding:
                    ...
                    output = self.reward_module(
                        input_ids=input_ids_rmpad, attention_mask=None, position_ids=position_ids_rmpad, **multi_modal_inputs, use_cache=False
                    )
...
#                     else:
                ...
                return log_probs #(batch_size, seq_len, vocab_size)
        else:
...
#         rm_multi_modal_inputs = data.non_tensor_batch["multi_modal_inputs"]

#         rm_inputs = {"input_ids": rm_input_ids, "attention_mask": rm_attention_mask, "position_ids": rm_position_ids, "multi_modal_inputs": rm_multi_modal_inputs}
...
#                 "multi_modal_inputs": rm_multi_modal_inputs
#             }
...
#         rm_data.batch = rm_data.batch.to(get_device_id())
        ...
        with self.ulysses_sharding_manager:
            ...
            if self.config.on_policy_distill:
                teacher_log_logits = scores  # (bs, response)
                # Note that this is only the scores, may not be the final rewards used to train RL
                output = DataProto.from_dict(tensors={"teacher_log_prob": teacher_log_logits})
                output = self.ulysses_sharding_manager.postprocess_data(data=output)
            else:
                token_level_scores = self._expand_to_token_level(data, scores)
                # Note that this is only the scores, may not be the final rewards used to train RL
                output = DataProto.from_dict(tensors={"kl_scores": token_level_scores})
                output = self.ulysses_sharding_manager.postprocess_data(data=output)
