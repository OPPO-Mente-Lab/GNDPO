...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        if "teacher_log_prob" in data.batch.keys():
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    if self.config.use_kl_loss_teacher:
                        teacher_log_prob = data["teacher_log_prob"]
                        # compute kl loss
                        kld = kl_penalty(
                            logprob=log_prob, ref_logprob=teacher_log_prob, kl_penalty=self.config.kl_loss_type
                        )
                        kl_loss = agg_loss(loss_mat=kld, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)

                        policy_loss = policy_loss + kl_loss * self.config.kl_loss_coef_teacher
                        micro_batch_metrics["actor/kl_loss_teacher"] = kl_loss.detach().item()
                        micro_batch_metrics["actor/kl_coef_teacher"] = self.config.kl_loss_coef_teacher
