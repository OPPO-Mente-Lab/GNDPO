...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        grpo_calculation_mask = data.batch["response_mask"]
        # Call compute_grpo_outcome_advantage with parameters matching its definition
        advantages, returns = core_algos.compute_reverse_kl_and_adv(
        ...
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.ON_POLICY_DISTILL_NORM:
        grpo_calculation_mask = data.batch["response_mask"]
        # Call compute_grpo_outcome_advantage with parameters matching its definition
        advantages, returns = core_algos.compute_reverse_kl_and_adv_norm(
            teacher_log_prob=data.batch["teacher_log_prob"],
            student_log_prob=data.batch["old_log_probs"],
            response_mask=grpo_calculation_mask,
        )
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if self.config.algorithm.adv_estimator == AdvantageEstimator.GAE:
            ...
            AdvantageEstimator.ON_POLICY_DISTILL_NORM
        ]:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if rollout_data_dir:
                        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
                            ...
                            inputs = [query.replace("<｜end▁of▁sentence｜>", "") for query in inputs]
                            ...
                            outputs = [response.replace("<｜end▁of▁sentence｜>", "") for response in outputs]
