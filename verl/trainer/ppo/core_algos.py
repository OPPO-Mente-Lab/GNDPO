...
class AdvantageEstimator(str, Enum):
    ...
    ON_POLICY_DISTILL_NORM = "on_policy_distill_norm"
...
@register_adv_est(AdvantageEstimator.ON_POLICY_DISTILL)  # or simply: @register_adv_est("on_policy_distill")
def compute_reverse_kl_and_adv(
    teacher_log_prob: torch.Tensor,
    student_log_prob: torch.Tensor,
    response_mask: torch.Tensor,
):
    """
    Compute advantage for ON-POLICY DISTILL
    ...
    with torch.no_grad():
        kl_scores = teacher_log_prob - student_log_prob
        kl_scores = torch.clamp(kl_scores, min=-5.0, max=5.0)
        kl_adv = kl_scores * response_mask
    return kl_adv, kl_adv

@register_adv_est(AdvantageEstimator.ON_POLICY_DISTILL_NORM)
def compute_reverse_kl_and_adv_norm(
    teacher_log_prob: torch.Tensor,
    student_log_prob: torch.Tensor,
    response_mask: torch.Tensor,
):
    """
    Compute advantage for ON-POLICY DISTILL NORM
    ...
    with torch.no_grad():
        kl_scores = teacher_log_prob - student_log_prob
        kl_scores = torch.clamp(kl_scores, min=-5.0, max=5.0)
        kl_adv = verl_F.masked_whiten(kl_scores, response_mask) * response_mask
    return kl_adv, kl_adv
