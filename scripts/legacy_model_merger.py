...
class BaseModelMerger(ABC):
    def __init__(self, config: ModelMergerConfig):
        ...
        print(f'BaseModelMerger config: {config}')
        self.model_config = AutoConfig.from_pretrained(self.hf_model_config_path, trust_remote_code=config.trust_remote_code)

    def get_transformers_auto_model_class(self):
        if "AndesVL" in self.model_config.architectures[0]:
            return AutoModelForCausalLM
        elif "ForTokenClassification" in self.model_config.architectures[0]:
...
def main():
    ...
    merge_parser.add_argument(
        "--trust_remote_code", default=False, type=bool, help="Hugging Face repository ID to upload the model"
    )
